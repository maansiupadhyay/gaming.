...
function NoAnswered() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const url = `http://localhost:8080/no_answered?user_id=${auth.user_id}&page_no=${pageNo}&limit=${questionsPerPage}`;
  ...
  const startQuestion = (pageNo - 1) * questionsPerPage + 1;
      ...
      <div style={{ display: "flex" }}>
        <div style={{ flex: 2, marginTop: 10 }}>
              ...
              <div className="question-content">
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                  <div style={{ display: "flex" }}>
                    ...
                    <p style={{ fontSize: 22, fontWeight: "400", marginTop: 10, marginLeft: 5 }}>
                      {question.username}
                    </p>
                  </div>
          ...
          <div style={{ display: "flex", justifyContent: "center", marginTop: 20 }}>
            ...
            <PaginationButton onClick={handleNextPage} disabled={startQuestion+10 >= totalQuestions}>
              Next
            </PaginationButton>
          </div>
          <div style={{ textAlign: "center", marginTop: 10 }}>
            <p>
              Showing questions {startQuestion} to {startQuestion+10} of {totalQuestions} total questions
            </p>
          </div>
        </div>
